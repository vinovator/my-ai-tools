...
- **Grouped portal, latest-first within a group:** the root `index.html` portal splits tools into three `<section>` blocks — Finance and valuation, Learning and visualisation, Utilities. When adding a new tool, insert its card at the **start of the section it belongs to**, not at the top of the page. Each card is one link: the anchor sits on the `<h3>` title and is stretched over the whole card with `::after`, and the visible `Open` cue is a `<span>`, never a second link. Card colour comes from `style="--card-accent: var(--accent-<tool>)"`, and the variable must be declared in both the `:root` and `:root[data-theme="light"]` palettes unless the dark value already reads well on white.
